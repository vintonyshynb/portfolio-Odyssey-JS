...
const icons = {
    shield: textureLoader.load('img/shield.png'),
    slow: textureLoader.load('img/slow.png'),
    double: textureLoader.load('img/double.png'),
    invert: textureLoader.load('img/invert.png'),
    speed: textureLoader.load('img/speed.png'),
    shake: textureLoader.load('img/shake.png')
}
...
loader.load('/glb/spaceship.glb', (gltf) => {
    player = gltf.scene
    scene.add(player)
...
loader.load('/glb/quaternius_cc0-spaceship-1367.glb', (gltf) => {
    player2 = gltf.scene
    scene.add(player2)
    ...
    if (gameStarted && !paused && !gameOver && player) {
        ...
        if (gameMode === "invader") {
            if (score % 200 == 0 && Math.floor(score / 200) > Math.floor(lastBossSpawnScore / 200) && !boss) {
                loader.load('/glb/plaggy_cc0-ufo-621.glb', (gltf) => {
                    boss = gltf.scene
                    scene.add(boss)
